...
bool Object::parse(std::istream& input, Object& object, bool root) {
    object.reset();
...
bool Object::parse(std::istream &input, bool root) {
  return parse(input,*this, root);
}
bool Object::parse(const std::string &input) {
  std::istringstream is( input );
  return parse(is,*this, true);
}
