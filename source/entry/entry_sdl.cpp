...
#	endif // BX_PLATFORM_
		...
		struct Context
		{
			...
			void run(int _argc, char** _argv)
			{
				...
				set_window_size(defaultWindow, m_width, m_height, true);
					...
					while (SDL_PollEvent(&event) )
					{
						...
						case SDL_WINDOWEVENT:
						{
												...
												case SDL_WINDOWEVENT_SIZE_CHANGED:
												{
																					 ...
																					 set_window_size(handle, wev.data1, wev.data2);
												}
													break;
						...
						default:
						{
								   ...
								   case SDL_USER_WINDOW_SET_SIZE:
								   {
																	...
																	if (is_valid(handle) )
																	{
																		set_window_size(handle, msg->m_width, msg->m_height);
																	}
																	delete msg;
								   }
									   break;
			...
			void set_window_size(WindowHandle _handle, uint32_t _width, uint32_t _height, bool _force = false)
			{
				if (_width  != m_width
					||  _height != m_height
					||  _force)
